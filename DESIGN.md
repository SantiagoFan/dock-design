...
排版构建整体骨架。PingFang SC（系统优先，Web 字体 Noto Sans SC 兜底）承担中文显示，Inter 承担英文，Geist Mono 承担数字与代码。大标题 32-40px 字重 600、行高 1.25、负字距 -0.01em；正文 14px 字重 400、行高 1.6。

色彩由 MindLink 蓝 `#3662EC` 主线驱动，仅出现在交互元素上。背景三层：`#F7F9FD`（页面）/ `#FFFFFF`（卡片）/ `#F5F8FF`（品牌区）。语义色仅用于状态反馈。

**Key Characteristics:**
- PingFang SC（系统优先）+ Inter + Geist Mono 三通道排版
...
**Font Stack**（v2.0 调整 · 2026-04-29）
- 中文（**系统字体优先**）：`"PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Noto Sans SC", sans-serif`
- 英文：`"Inter", -apple-system, BlinkMacSystemFont, sans-serif`
- 数字/代码：`"Geist Mono", "JetBrains Mono", "SF Mono", ui-monospace, Menlo, monospace`

**说明**：
1. **中文系统字体优先**——macOS / iOS 用户直接走 `PingFang SC`（Apple 优雅字体，零网络成本）；Windows 用户走 `Microsoft YaHei`；Web 字体 `Noto Sans SC` 作为最终兜底。这与 Element Plus / Ant Design / 国内主流 SaaS 默认策略一致，避免不必要的 woff2 下载。
...
- 字体三通道: PingFang SC（系统优先）/ Inter / Geist Mono
...
3. 字体三通道: PingFang SC（系统优先）/ Inter / Geist Mono
