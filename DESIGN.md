...
> Geist Mono / JetBrains Mono 等等宽字体**不内置中文字形**，强制使用会回退到操作系统默认中文字体（Windows 上常是宋体细体），视觉断层。KPI 数字 + 中文单位的写法：
> ```css
> .kpi-num { font-family: var(--du-font-mono); }      /* 父：Mono · 数字等宽 */
> .kpi-num .unit { font-family: var(--du-font-sans); } /* 子单位：Sans · 中文用 */
> ```
> ```html
> <div class="kpi-num">8<span class="unit">个</span></div>     <!-- ✅ -->
> <div class="kpi-num">186<span class="unit">ms</span></div>   <!-- ✅ ms 仍走 sans 但与父级数字无视觉断层 -->
> ```
