...
排版构建整体骨架。Noto Sans SC（Web 优先，PingFang SC / Microsoft YaHei 兜底）承担中文显示，Inter 承担英文 + 数字（含 tabular-nums 等宽数字），双通道协同。大标题 32-40px 字重 600、行高 1.25、负字距 -0.01em；正文 14px 字重 400、行高 1.6。

色彩由 MindLink 蓝 `#3662EC` 主线驱动，仅出现在交互元素上。背景三层：`#F7F9FD`（页面）/ `#FFFFFF`（卡片）/ `#F5F8FF`（品牌区）。语义色仅用于状态反馈。

**Key Characteristics:**
- Noto Sans SC（Web 优先）+ Inter（英文 / 数字）双通道排版
...
- **数字 / 代码（Inter 统一）**：`"Inter", -apple-system, BlinkMacSystemFont, "SF Mono", ui-monospace, Menlo, monospace`
  - `--du-font-mono` 不再加载独立 mono 字体，而是复用已加载的 Inter
  - 等宽对齐通过 `font-variant-numeric: tabular-nums` 实现（Inter 原生支持）

**说明**：
1. **中文 Web 字体优先**——`Noto Sans SC`（Google 出品，OFL 开源，Variable Font）作为头位，**保证 Mac / Windows / Linux 所有平台视觉完全一致**——这对设计师跨设备评审、产品截图归档、用户口碑统一至关重要；如 Web 字体加载失败，降级到 macOS/iOS 的 `PingFang SC` → 老 Mac 的 `Hiragino Sans GB` → Windows 的 `Microsoft YaHei`。
2. **数字字体统一 Inter（v2.0 重要决策 · 2026-04-30）**——
   - **不再用 Geist Mono / JetBrains Mono 等独立 mono 字体**。原因：
     - ❌ 独立 mono 字体（Geist / JetBrains）只有英文字形，**中文混排时回退到 OS 默认（Windows 宋体细体），视觉灾难**
     - ❌ Google Fonts CDN 在国内不稳定，CDN 加载失败时 mono 字体退化为浏览器默认 Courier 系，跨端不一致
     - ❌ 项目实际很少有真正的"代码显示"场景（终端 / 编辑器），仅有少量 token 名、时间戳、KPI 数字
   - **Inter 完美胜任**：
     - ✅ 已为英文加载，零额外 CDN 请求
     - ✅ Inter 数字辨识度高（`0` / `O`、`1` / `l` / `I` 都明显区分）
     - ✅ 配 `font-variant-numeric: tabular-nums` 数字等宽对齐，效果≈mono
     - ✅ 与 Noto Sans SC 同回退栈，中文 unit（个/位/单）混排零问题
     - ✅ Linear / Notion / Figma / Stripe 等主流 SaaS 都这么做
3. **Inter 保留**——已是 OFL 开源、Variable Font、SaaS 行业标杆，无需替换。

**KPI 数字标准写法**：
```css
.kpi-num {
  font-family: var(--du-font-mono);     /* 解析为 Inter */
  font-variant-numeric: tabular-nums;   /* 数字等宽 · 关键 */
  letter-spacing: -0.01em;              /* 紧凑感 */
  font-weight: var(--du-fw-semibold);
}
.kpi-num .unit {
  font-family: var(--du-font-zh);       /* 单位用中文友好栈 */
  font-size: var(--du-fs-md);           /* 67% 比例 */
  font-weight: var(--du-fw-regular);
  color: var(--du-text-3);
}
```
...
> **⚠️ Mono 字体内嵌中文时，中文部分必须切回 sans 字体**
> v2.0 起 `--du-font-mono` 已统一映射到 Inter（不再用独立 mono 字体），所以下方"中文 unit 切回 sans"在 Inter 时代变成"显式声明语义"——实际两者都是 Inter / Noto Sans 同一栈，但显式注明 `--du-font-zh` 仍是好习惯。原 Geist Mono / JetBrains Mono 等独立 mono 字体**不内置中文字形**导致的字体回退灾难已根治。
>
> **附加规则**：单位字号必须与父级数字保持 **65~75% 比例**（避免下标视觉），原因：
> - 拉丁字符（`GB` `ms` `%`）有自然 x-height，小字号下视觉仍均衡
> - 中文方块字（`个` `位` `单`）填满 em-square，过小会视觉退化为下标
>
> **标准比例对照**：
> | 父级 (.kpi-num / .stat-num) | 单位 (.unit) | 比例 |
> |---|---|---|
> | `--du-fs-3xl` (32px) | `--du-fs-lg` (18px) | 56% |
> | `--du-fs-2xl` (24px) | `--du-fs-md` (16px) | 67% |
> | `--du-fs-xl` (20px) | `--du-fs-md` (16px) | 80% |
>
> ```css
> .kpi-num { font-family: var(--du-font-mono); font-size: var(--du-fs-2xl); }
> .kpi-num .unit {
>   font-family: var(--du-font-sans);   /* 切回 sans · 中文友好 */
>   font-size: var(--du-fs-md);          /* 67% 比例 · 中英文都协调 */
>   font-weight: var(--du-fw-regular);
>   color: var(--du-text-3);
> }
> ```
> ```html
> <div class="kpi-num">8<span class="unit">个</span></div>     <!-- ✅ -->
> <div class="kpi-num">186<span class="unit">ms</span></div>   <!-- ✅ ms 也走 sans，视觉一致 -->
> ```
...
- Value: 28-32px Inter weight 600 + `font-variant-numeric: tabular-nums`，color `--du-text-1`
...
- 字体双通道: Noto Sans SC（Web 优先）/ Inter（英文 + 数字）
...
- **KPI Card**: "#FFFFFF bg, 12px radius, 24px padding, shadow 0 4px 12px rgba(15,27,60,.06). Label 12px #6B7A9C. Number 32px Inter weight 600 + tabular-nums #0F1B3C. Trend tag #EFFFEB bg #2DAC0C text. 入场 stagger slide-up 200ms spring, 数字 count-up 1200ms."
...
- **Table**: "Header: #F7F9FD bg, Inter 12px uppercase #3A4868. Body: 48px row, 13px, border-bottom #EEF2FA. Hover: #F5F8FF. Selected: #EAEFFD + inset 3px #3662EC. Numeric: Inter + tabular-nums right-aligned. Pagination 28px btns."
...
3. 字体双通道: Noto Sans SC（Web 优先）/ Inter（英文 + 数字）
